...
namespace TekstilScada.Core
{
    public static class RecipeCostCalculator
    {
        ...
        public static (decimal TotalCost, string CurrencySymbol, string Breakdown) Calculate(ScadaRecipe recipe, List<CostParameter> costParams)
        {
            ...
            var AirParam = costParams.FirstOrDefault(p => p.ParameterName == "Air");
            ...
            if (AirParam == null || electricityParam == null || steamParam == null)
            {
                return (0, "TRY", "Missing cost parameters.");
            }

            decimal totalAirLiters = 0;
            ...
            foreach (var step in recipe.Steps)
            {
                ...
                if ((controlWord & 1) != 0) totalAirLiters += step.StepDataWords[1];
            ...
            decimal AirCost = totalAirLiters * AirParam.CostValue * AirParam.Multiplier;
            ...
            decimal totalCost = AirCost + electricityCost + steamCost;
            string currencySymbol = AirParam.CurrencySymbol ?? "TRY";

            // Create a detailed breakdown text
            string breakdown = $"Air: {AirCost:F2} {currencySymbol}\nElectricity: {electricityCost:F2} {currencySymbol}\nSteam: {steamCost:F2} {currencySymbol}";
